...
\chapter{Resultados}\label{chap:resultados}
	...
	Com o intuito de facilitar a comparação, os resultados das Tabelas \ref{tab:media_desvio_historico} e \ref{tab:media_variancia_resultado} foram reprentados graficamente na Figura \ref{fig:comparativo_agrupado}.
	...
	\begin{axis}[
		ybar,
		bar width=15pt,
		symbolic x coords={Automação,Banco de dados,Relatório de dados,Script,Tela},
		xtick=data,
		enlarge x limits=0.2,
		ylabel={Pontos por Tarefa},
		ymin=0,
		grid=major,
		legend style={at={(0.65,0.95)},anchor=north},
		width=0.95\textwidth,
		height=0.4\textheight,
		xticklabel style={rotate=30},
	]

	% Barras agrupadas mais compactas
	\addplot+[bar shift=-7.5pt, fill=skyblue] coordinates {
		(Automação,4.52)
		(Banco de dados,4.00)
		(Relatório de dados,3.14)
		(Script,2.80)
		(Tela,4.13)
	};
	\addplot+[bar shift=-7.5pt, fill=deepskyblue] coordinates {
		(Automação,1.44)
		(Banco de dados,2.71)
		(Relatório de dados,0.55)
		(Script,0.91)
		(Tela,1.85)
	};
	\addplot+[bar shift=7.5pt, fill=lightcoral] coordinates {
		(Automação,1.38)
		(Banco de dados,1.56)
		(Relatório de dados,2.63)
		(Script,2.67)
		(Tela,2.00)
	};
	\addplot+[bar shift=7.5pt, fill=indianred] coordinates {
		(Automação,0.57)
		(Banco de dados,0.07)
		(Relatório de dados,0.28)
		(Script,0.00)
		(Tela,0.51)
	};

	\legend{
	Média (Antes), Desvio (Antes),
	Média (Depois), Desvio (Depois)
	}
	\end{axis}
	\end{tikzpicture}
	\caption{Comparação agrupada de média e desvio padrão por categoria, antes e depois das mudanças}
	\label{fig:comparativo_agrupado}
	\end{figure}

	É notável, ao observar a Figura \ref{fig:comparativo_agrupado}, que houve uma queda nas médias de pontos por tarefa em todas as categorias de desenvolvimento, ao se comparar com as mesmas estatísticas dos dados históricos.
	...
	Observa-se um bom aproveitamento das tarefas sugeridas, e nota-se que, para soluções menos complexas, o agente autônomo foi mais assertivo nas tarefas a serem executadas. Esse tipo de
	...
	Foi feito ainda um gráfico mostrado na Figura \ref{fig:grafico_cenarios}, que facilita essa comparação.
	...
	\begin{figure}[!htb]
		...
		\begin{axis}[
			ybar,
			bar width=15pt,
			symbolic x coords={Cenário 1,Cenário 2,Cenário 3},
			xtick=data,
			enlarge x limits=0.3,
			ylabel={Esforço (Pontos)},
			ymin=0,
			grid=major,
		...
		\end{axis}
		\end{tikzpicture}
		\caption{Comparativo da média e desvio padrão de esforço entre os três cenários, antes e depois da análise pelo aplicativo}
		\label{fig:grafico_cenarios}
	\end{figure}

	Apesar de indicado o desvio padrão para as medidas, não se pode atribuir grande importância a essa comparação, visto que foram poucas amostragens de esforço devido ao pequeno número de desenvolvedores e cenários.
